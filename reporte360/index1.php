...
function convertir($srt){
	$array=array("ó"=>"&oacute;","ñ"=>"&ntilde;","á"=>"&aacute;","é"=>"&eacute;","í">="&iacute;");
 	...
 	for($i=1;$i<=$hojas;$i++){  
		...
		//echo "<pre>";print_r($data["report"]);echo "</pre>";exit;	    	
			...
			foreach(json_decode($data["report"]->escalas->jsondesccriptor) as $k=>$d){
				...
				$html.='<td style="width:200px;height:40px;font-size:14px;border-collapse: collapse;border:#000 1px solid;padding-left:10px">';
				$html.=  convertir($data["report"]->rel[$k]->nombre);
				$html.='</td>';
				$html.='<td style="width:40px;height:40px;font-size:14px;background-color:#F2F2F2;color:#000;text-align:center;border-collapse: collapse;border:#000 1px solid;">';
			...
			$footer='<div style="z-index:999;margin-top:-72px;text-align:right;font-family: roboto !important;font-size:14px;font-style:normal !important">pág ';
			$footer.=$i;
			$footer.='</div>';
				...
				foreach($data["report"]->preguntasxcompetencias->{$dataA->keyid} as  $datac){ 
 					if($contadorpreguntas<=4 ){
 						if(!in_array($datac->keyidpregunta,$control)){
							...
							if(count($temporal)==4  or count($temporal)==$totalpreg){
				...
				$html=' <div id="table" style=" width:756px;  font-family: roboto;z-index:999;height:962px;  " ><br><br>    ';
						...
						$html=' <div id="table" style=" width:756px;   font-family: roboto;z-index:999;height:962px;   " >
								...
								$html.='<table cellspacing=0     style="border-collapse: collapse;">
									...
									$html.='<td width="250">';
										...
										<tr>';
										$html.='<td style="text-align:center;border-collapse: collapse;border:#000 1px solid;height:40px;padding:10px;font-family:\'Roboto Ligth\'" ><span>'.convertir($dataA->competencia);
										$html.='</span></td>
										</tr>
										</table>
									</td>
									...
									<tr>
										...
										<td>    
											...
											<tr>
												...
												<td  width="70" style="background-color:#D3D2D1;border-collapse: collapse;border:#000 1px solid;color:#000;text-align:center;font-size:13px"> 3.49 - 3.89 
												</td>
												<td  width="70" style="background-color:#A9F5BC;border-collapse: collapse;border:#000 1px solid;color:#000;text-align:center;font-size:13px"> 3.89 - 4.3 
												</td>
					...
					foreach( $tdatac as  $datac){
							...
							$html.='<tr>'; 
								...
								$html.='<td style="border-collapse: collapse;border:#000 1px solid;padding:15px;font-family:\'Roboto Ligth\';width:250px"><span>'.$numpre.". ".convertir($datac->enunciado);
								$html.='</span></td>';
											...
											$html.='<table style="width:69px;border-collapse: collapse;">';
											 foreach($datac->respuesta as $relaciones=>$varc){  
												...
												if($varc<3.1){
													...
													$html.='<td style="border-collapse: collapse;border:#000 1px solid;height:25px;width:48px;color:#fff;height:25px">0000';
													$html.='</td>';
												}
											 $html.='</tr>';
											 }
											 ...
											 if($datac->promedio<3.1){
												...
												$html.='<td style="border-collapse: collapse;border:#000 1px solid;width:48px;color:#fff;height:25px">0000';
												$html.='</td>';
											 }
											 $html.='</tr>';
											 $html.='</table>';								
										...
										$html.='<td  width="70"  >';
											...
											$html.='<table  style="width:69px;border-collapse: collapse;" >';
											 ...
											 if($datac->promedio>=3.49 and $datac->promedio<3.89){
												...
												$html.='<td style="border-collapse: collapse;border:#000 1px solid;width:48px;width:48px;color:#fff;height:25px">0000';
												$html.='</td>';
											 }
