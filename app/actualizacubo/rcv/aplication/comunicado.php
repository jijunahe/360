...
class Action{
 	...
 	public function __construct()
	{
		...
		if(isset($_GET["indicador"])){
			$this->indicador=json_decode($_GET["indicador"]);
		}
		 
		if(isset($_GET["relojes"])){
			$this->relojes=json_decode($_GET["relojes"]);
		}
		...
		if(isset($_GET["idAnswere"])){
			$this->idAnswer=$_GET["idAnswere"];
		}
		 
		if(isset($_GET["observaciones"])){
			$this->observaciones=$_GET["observaciones"];
		}
		 
		if(isset($_GET["titulo"])){
			$this->titulo=json_decode($_GET["titulo"]);
		}
	...
	public function init(){     
		...
		$results =$this->bdObj->select(PREFIJO_TABLAS."cubo_425936","idAnswer='".$this->idAnswer."'");
		$info=$results[0];		
		
 		$info["425936X35X1154"]=$this->email;
 		$info["425936X35X1158"]=$this->email;
		$imagenes=array();
		
		foreach($this->titulo as $k=>$dato){
			$code=base64_encode(json_encode($this->relojes[$k]));
			array_push($imagenes,"<img src='https://".$_SERVER["SERVER_NAME"]."/fcc/jpgraph/index.php?titulo=".$dato."&puntaje=".$this->indicador[$k]."&relojes=".$code."' />");
			//array_push($imagenes,"");
  		}
		...
		$html.='</tr>';
 		$html.='</table><br>'; 
 				...
 				$valida=array("425936X35X1152","425936X35X1153","425936X35X1154","mor10aniosNoCo","mor10aniosT","mor10aniosCo","token","lastpage","startlanguage","startdate","datestamp","encuesta","rangoedad","idEncuesta","encuesta");
					...
					"indice"=>"&Iacute;ndice de masa corporal",
					...
					"425936X35X1175"=>"&iquest;Practicas alguna actividad f&iacute;sica?",
					"edad"=>"Edad",
					"425936X35X1168"=>"&iquest;Eres fumador?",
					"425936X35X1169"=>"&iquest;Sufre de hipertensi&oacute;n Arterial?",
					"425936X35X1170"=>"Tensi&oacute;n Arterial Sist&oacute;lica",
					"425936X35X1171"=>"Tensi&oacute;n Arterial Diast&oacute;lica",
					"425936X35X1178"=>"&iquest;Alguna vez el m&eacute;dico le ha dicho que usted, tiene un problema card&iacute;aco y que por eso s&oacute;lo deber&iacute;a realizar actividad f&iacute;sica recomendada por &eacute;l?",
					"425936X35X1179"=>"&iquest;Cu&aacute;ndo hace actividad f&iacute;sica siente dolor en el pecho?",
					"425936X35X1180"=>"&iquest;En el &uacute;ltimo mes y estando en reposo, ha sentido dolor en el pecho?",
					"425936X35X1181"=>"&iquest;Pierde el equilibrio por mareos o v&eacute;rtigo, o alguna vez ha perdido el conocimiento?",
					"425936X35X1182"=>"&iquest;Tiene un problema &oacute;seo o articular que pudiera empeorar por un aumento en su actividad f&iacute;sica habitual?",
					"425936X35X1183"=>"&iquest;Actualmente el m&eacute;dico le est&aacute; prescribiendo medicamentos (por ejemplo diur&eacute;ticos) para su presi&oacute;n arterial o para su coraz&oacute;n?",
					"425936X35X1184"=>"&iquest;Conoce&ensp;alguna otra raz&oacute;n&ensp;por la cual no deber&iacute;a hacer actividad f&iacute;sica?",
					"parqu"=>"Par q & u",
					"425936X35X1172"=>"&iquest;Presentas condiciones cardiovasculares?",
					"425936X35X1174"=>"&iquest;Sufres de diabetes?",
			...
			switch($key){
				...
				case "control":
					...
					if($valor[0]<=9.9){ 
						...
						$gDias=365;
						$accion='Mantener manejo, retroalimentar y estimular&ensp;(EVS y tratamiento)';
					}
