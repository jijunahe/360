                ...
                if (isset($_SESSION[$this->sessid]['srid']) && $this->surveyOptions['active'])
                {
                    ...
                    else
                    {
                        if ($finished) {
							...
							$IDS=$_SESSION[$this->sessid]['srid'];
                            dbExecuteAssoc($sQuery);   // Checked
							
							$queryc = "SELECT  * FROM ".$this->surveyOptions['tablename']." WHERE id=".(INT)$IDS;
							$gcrt = dbExecuteAssoc($queryc);
							$resgcrt = $gcrt->readAll();
 							 
							list($pre,$cod)=explode("_",$this->surveyOptions['tablename']);
							list($cod,$n)=explode("}}",$cod);							
 							
							$documentroot=str_replace("index.php","",$_SERVER["SCRIPT_FILENAME"]);
							include($documentroot.'application/rules/'.$cod.'.php');
                          }
                    }
