...
class Felicidad extends Survey_Common_Action
{
	public $anioc=array(); 
	public $id=7;
	public $sid="214322";
    ...
    {   Yii::app()->cache->flush();
		...
		if(isset($dUsuario->id)){
			...
			$criteria->condition = ' idorg='.$dUsuario->id_unidad.' and idmodulo='.$this->id;
			$hseqdisp=Encuestalibreparams::model()->findAll($criteria);
 		}
		
		$data["hseqdisp"]=$hseqdisp;
 		
		$this->_renderWrappedTemplate('felicidad', 'index',$data);
    }
	...
	public function action(){
		...
		if(isset($_POST["option"])){
			...
			switch($option){
				case "create":
					...
					$documentos=explode(",",$_POST['Felicidad']["documentos"]);
					unset($_POST['Felicidad']["documentos"]);
					$model->token=md5(date("Y-m-d H:i:s"))."::214322";
					$model->idorg=$dUsuario->id_unidad;
					$model->attributes=$_POST['Felicidad'];
 				...
 				break;
				case "update":
  					$model=Encuestalibreparams::model()->findByPk((int)$_POST['Felicidad']["id"]);
					if(isset($model->id)){
						if($model->id>0){
							$documentos=explode(",",$_POST['Felicidad']["documentos"]);
							unset($_POST['Felicidad']["documentos"]);
		...
		$this->getController()->redirect(array('admin/felicidad/','r'=>$error)); 
	}
	...
	public function reportes(){
		...
		if(isset($dUsuario->id)){
			...
			if(!isset($_POST["chart"])){
				$this->_renderWrappedTemplate('felicidad', 'reportes', $data);
			}else{
