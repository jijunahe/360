		...
		//printVar($oRecord);exit;
  		...
  		if ($oRecord->uid>1){ 
			$aData["per"]=1;
 			...
 			$aData["imageurl"]="/".$ruta[1]."/img/";
			$aData["rutabase"]="/".$ruta[1]."/index.php/";
			$aData["rutasurvey"]="/".$ruta[1]."/index.php/survey/index/sid/";
			...
			$aData["anio"]=$anios;			
  			...
  			if($oRecord->iduseval>0){
 				$dato = EvalUsuarios::model()->findByPk($oRecord->iduseval);
				 
				 if($dato->perfil==3 or $dato->perfil==6 or $dato->perfil==7 or $dato->perfil==2){
					$aData["u"]="";
					//$this->_renderWrappedTemplate('survey', 'administrador', $aData);
					$this->_renderWrappedTemplate('survey', 'docspcos', $aData);
				}
				 if($dato->perfil==4){
					$aData["u"]="";
 					$this->_renderWrappedTemplate('survey', 'docspcos', $aData);
 					//$this->_renderWrappedTemplate('survey', 'consultor', $aData);
				}//printVar($dato->perfil);exit;
				 if($dato->perfil==5){
					$aData["u"]="";
					$this->getController()->redirect(array('/admin/hseq/encuesta'));	
 				}
 			}else{
