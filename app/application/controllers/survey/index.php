...
class index extends CAction {
    ...
    function action()
    { //echo Yii::app()->request->scriptFile;exit;
        global $surveyid;
		...
		$cs->registerScriptFile($baseUrl.'/js/jquery.csv-0.71.js');
		/*
		$cs->registerScriptFile('//ajax.googleapis.com/ajax/libs/jquery/1.12.4/jquery.min.js');
		...
		*/
		...
		/*
 		$nombre_archivo="/var/www/hseq/cookie/". $clienttoken;
		if(isset($_GET["rf"])){
			echo $clienttoken;
 			if(!file_exists($nombre_archivo))
			{
				...
				{
					fwrite($archivo, trim($_GET["rf"]));
					fclose($archivo);
					}		 
			 
			} 
		} 		
 		if(file_exists($nombre_archivo) and !isset($_GET["rf"])) 
		{
			echo nl2br(file_get_contents($nombre_archivo));
			setCookie("referencia",nl2br(file_get_contents($nombre_archivo)));
