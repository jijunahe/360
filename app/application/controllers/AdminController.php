...
class AdminController extends LSYii_Controller
{
    ...
    */
    protected function _init()
    {
	  ...
	  //  printVar(Yii::app()->user->id);exit;
		$idusval=User::model()->findByPk(Yii::app()->user->id);
		$dataus=NULL;
		if(isset($idusval->iduseval)){
			$dataus=AnaUsuario::model()->findByPk($idusval->iduseval);
		}
		//printVar(Yii::app()->user->id);exit;
 		/*if($dataus->perfil>1 and (int)($idusval->iduseval)>0){
				//	App()->getClientScript()->registerCssFile(Yii::app()->baseUrl . "/scripts/hyperspace/assets/css/main.css" );
		} 
		*/ 
		 
	    App()->getClientScript()->registerCssFile(Yii::app()->baseUrl . "/scripts/bootstrap2018/vendor/bootstrap/css/bootstrap.min.css" );
 	    App()->getClientScript()->registerCssFile(Yii::app()->baseUrl . "/scripts/bootstrap2018/vendor/metisMenu/metisMenu.min.css" );
 	    App()->getClientScript()->registerCssFile(Yii::app()->baseUrl . "/scripts/bootstrap2018/dist/css/sb-admin-2.css" );
 	    App()->getClientScript()->registerCssFile(Yii::app()->baseUrl . "/scripts/bootstrap2018/vendor/morrisjs/morris.css" );
 	    App()->getClientScript()->registerCssFile(Yii::app()->baseUrl . "/scripts/bootstrap2018/vendor/font-awesome/css/font-awesome.min.css" );
	   ...
	   $sUpdateLastCheck = getGlobalSetting('updatelastcheck');
		...
		if(is_object($dataus)){
			if($dataus->perfil>1  and (int)($idusval->iduseval)>0){
				/*App()->getClientScript()->registerScriptFile(Yii::app()->baseUrl .  "/scripts/hyperspace/assets/js/jquery.scrollex.min.js" );
				App()->getClientScript()->registerScriptFile(Yii::app()->baseUrl .  "/scripts/hyperspace/assets/js/jquery.scrolly.min.js" );
				App()->getClientScript()->registerScriptFile(Yii::app()->baseUrl .  "/scripts/hyperspace/assets/js/skel.min.js" );
				App()->getClientScript()->registerScriptFile(Yii::app()->baseUrl .  "/scripts/hyperspace/assets/js/util.js" );
				App()->getClientScript()->registerScriptFile(Yii::app()->baseUrl .  "/scripts/hyperspace/assets/js/main.js" );
				*/
			}
		}
    ...
    */
    ...
    public function getActionClasses()
    {
        ...
        'controladornodosreporte'=> 'controladornodosreporte',
        );
 		$controler=Controlador::model()->findAll();
		foreach($controler as $data){
			if(!in_array($data->archivo,$controladores)){
				$controladores[$data->archivo]=$data->clase;
			}
			
		}
	//printVar($controladores);exit;
    ...
    */
    public function _getAdminHeader($meta = false, $return = false)
    {
		...
		$usereval=NULL;
		$servicios=NULL;
		if(isset($usuario->iduseval)){
			$dataus=AnaUsuario::model()->findByPk($usuario->iduseval);
			$role=$dataus->perfil;
			$usereval=$dataus;
 		}
 
 
		//printVar($usereval); exit;
 
  		$aData["role"]=$role;
  		$aData["oRecord"]=$usuario;
  		$aData["usuario"]=$usereval;		
		//printVar($superMenu); 
 
        if (!empty(Yii::app()->session['dateformat']))
            $aData['formatdata'] = getDateFormatData(Yii::app()->session['dateformat']);

        $sOutput = $this->renderPartial("/admin/super/cabecera", $aData, true);
    ...
    */
    public function _showadminmenu($surveyid = false)
    {
		...
		$servicios=NULL;
		$modulos=NULL;
		if(is_object($usuario->iduseval)){
			if((int)$usuario->iduseval>0){
				$usereval=AnaUsuario::model()->findByPk($usuario->iduseval);
				$role=$usereval->perfil;
			}else if($usuario->uid==1){
				$role=1;
			}
		}
		 
		// printVar($superMenu); exit;
		
  		$aData["permisos"]=NULL;
  		$aData["servicios"]=$servicios;
  		$aData["superMenu"]=NULL;
  		$aData["role"]=$role;
  		$aData["oRecord"]=$usuario;
  		$aData["usuario"]=$usereval;
        $this->renderPartial("/admin/super/menu", $aData);

    }
	private function menuModulo($modulos,$role,$objuser=NULL){
 		$subQuery="";
 		if($modulos!=NULL){
			
			$subQuery=" and idmodulo in (".join(",",$modulos).") ";
		}
		$queryString = "
		SELECT 
		*
		FROM {{menuxmodulo}}
		WHERE estado='1' ".$subQuery." order by ordenid ASC";
		$eguresult = dbExecuteAssoc($queryString);
		$tmenu = $eguresult->readAll();			
 		$tempo=array();  		
		$superMenu=array();
  		foreach($tmenu as $datos){
			$datos=(object)$datos;
			$roles=explode(",",$datos->role);
			
 			if(in_array($role,$roles)  and $datos->padreid=="" ){
				$menu=array(
							"id"=>$datos->id,
							"idorden"=>$datos->ordenid,
							"titulo"=>$datos->titulo,
							"accion"=>$datos->accion,
							"submenu"=>""
							);
				$superMenu[$datos->id]=$menu;
 			}
			$tempo=$superMenu;
		} 
   		foreach($superMenu as $key=>$data){
			$submenu=array();
 			foreach($tmenu as $datos){
 				$datos=(object)$datos;
				$roles=explode(",",$datos->role);
				if(in_array($role,$roles) and $datos->padreid==$key){
					$criteria = new CDbCriteria;
					$criteria->condition ='padreid='.$datos->id;
					$criteria->order ="ordenid ASC";
					$sub=Menuxmodulo::model()->findAll($criteria);
					$submenu2=array();
					foreach($sub as $d){
						$r=explode(",",$d->role);
						if(in_array($role,$r)){
 							array_push($submenu2,(object)array("id"=>$d->id,"accion"=>$d->accion,"titulo"=>$d->titulo));
						}
					}
			...
			} //printVar($key,$objuser->id_unidad." unidad ");
			//
			if($key==43 and $objuser->id_unidad==27){}else{
				$superMenu[$key]["submenu"]=$submenu;
			}				
			//}
  		}		
		 
		foreach($superMenu as $key=>$modules){
			
			if($key==43 and $objuser->id_unidad==27){
