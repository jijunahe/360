...
?>
	...
	jQuery(function(){
		...
		jQuery("#aceptaterminos").click(function(){
			...
			var src="<?=$_SERVER["SCRIPT_NAME"]?>/<?=$sid?>/lang-es";
			var iframe='<iframe  id="encuesta"  name="encuesta" src="'+src+'?idempresa=<?=$org->id?>&idtoken=<?=$idtoken?>"  style="width:96%;height:95%;border: none; overflow-y:scroll" ></iframe>';
			//jQuery("#org").hide("slow");
			var test=(navigator.appVersion).split("iPhone");
			if(test[1]!=undefined){
				document.location=src+'?idempresa=<?=$org->id?>&idtoken=<?=$idtoken?>';
			}else{
...
<div id="welcome" class="divContentBlue" style="display:block">
		...
		<br>
		<!--
		<div   id="aviancahome"> <img src="<?=Yii::app()->baseUrl?>/images/Felicidad.png" width="100%"></div>
		<div   id="textohome">
			<p> 
				<b>No es lo mismo medir un Clima Organizacional que un Clima de Felicidad</b>
			</p>
			<p>
				Nos gustaría conocer tu nivel de felicidad Para esto a continuación te pedimos responder sinceramente la siguiente encuesta. Con esta información podremos formular estrategias para ayudarte a mantener una mejor calidad de vida y ser más feliz!
			</p>
		</div>
		-->
		<div id="descripcion">
			<p> 
				<b style="font-size:16px">Medir la felicidad resulta ser la mejor estrategia de Bienestar.</b>
			</p>
			 
			<p>Al finalizar la encuesta recibirás en tu correo electrónico un diagnóstico de qué tan feliz eres en este momento.
 			</p>
		</div>		
