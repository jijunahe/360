	...
	window["refreshIntervalId"] = setInterval(function(){
		jQuery("#IDS",encuesta.document).each(function(){         
			if(jQuery(this).attr('id')=="IDS"){
				...
				//jQuery("#iframe1").attr("src","https://www.talentracking.co/node/251?embed=1&token=<?=$token?>&idestructura=<?=$ides?>&paramidAnswer="+jQuery(this).val());
	...
	window["refreshIntervalIdautolinck"] = setInterval(function(){ 
		if(jQuery("#encuesta").size()>0){
			if(jQuery("#encuesta").contents().find("#completed").size()>=1){ console.log("test-listo");  
				...
				setTimeout(function(){
 					 jQuery("#pentaho").attr("src","https://www.talentracking.co/node/251?embed=1");
...
<? } ?>
<div id="content">
<input type="button" value="Cambiar área, dependencia, etc" rel="cambiar" />



  	<div class="divContentBlue">
		<h3>Herramienta RCV</h3>
		<? if($demo==false){ ?>
		<div class="divContentWhite">
			...
			<div class="divContentBlue rcv"  >
				...
				<iframe  id="pentaho"   name="pentaho" src="https://www.talentracking.co/node/251?embed=1&token=<?=$token?>&idestructura=<?=$ides2?><? if((int)$idAnswer>0){ echo "&paramidAnswer=".$idAnswer;} ?>"  style="width:96%;height:800px;border: none; overflow-y:scroll<? if($ides==NULL){echo "display:none";} ?>" ></iframe>
 				</div>
			</div>
			<div style="clear:both"></div>
		</div>
