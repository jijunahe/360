...
function ajustarbanner(){
 ...
 <div id="content">  
	...
	<img class="bannermodulo"  <?php if($demo==true){ ?>id="costos" style="cursor:pointer;width:100%;height:20%"<? } else{?> style="width:100%;height:20%" <? } ?> rel="radar.png"  />
	...
	<div class="divContentBlue"> 
		...
		<input class="link" style="float:left;margin-left:5px" type="button" value="Resultados consolidados" rel="https://www.talentracking.co/node/256?embed=1<? if($idestructura!=NULL){ ?>&idestructura=<? echo $idestructura; }?>&token=<?=$token?>"/>
		...
		<div class="divContentWhite">
			...
			?>
				...
				<?php echo CHtml::form(array("admin/rcv/resultadosconsolidados"), 'post', array('class'=>'acciones' , 'id'=>'acciones')); ?>
				...
				<?=$buscando?>					
				<iframe  id="reporte"   name="reporte" src="https://www.talentracking.co//node/256?embed=1&token=<?=$token?>&idestructura=<?=$idestructura?><?=$param?>"  style="width:100%;height:4000px;border: none; overflow-y:scroll<? if($ides==NULL){echo "display:none";} ?>" ></iframe>
