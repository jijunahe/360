...
<? 
	...
	if(count($totalnodos)>=1 and $simplificado=="N"){ ?>
 ...
 <? } ?>
	function event(){
		...
		jQuery("[rel^='estruc_']").click(function(){ 
			...
			var src="<?=$_SERVER["SCRIPT_NAME"]?>/<?=$sidid?>/lang-es?idestructura="+id+"&random=<?=$random?>";
...
<? }else if(count($totalnodos)==1 or   $simplificado=="Y"){  
	if($simplificado=="Y"){
		$ides=80144347;
	}else{
	$ides=$totalnodos[0];
...
function controlEncuesta(){	
		...
		if(jQuery("#encuesta").size()>0){
			jQuery("#completed",encuesta.document).each(function(){         
				if(jQuery(this).attr('id')=="completed"){ console.log("test-listo");  
					...
					setTimeout(function(){
						window.open ("http://corazonesresponsables.org","Corazones Responsables");
	...
	jQuery(function(){jQuery(".cargar").remove();})
...
<? } else{?>
<div  id="botonareas" style="display:none">
		<input type="button" value="Acepto terminos y condiciones" class="aceptaterminos" style="float:left" />
		<input   style="float:left;" type="button" class="verterminos" value="Ver terminos y condiciones"/>
</div>
...
<div id="textTerminos" style="display:none">
	...
	<div id="welcome" class="divContentBlue" style="display:block">
		...
		<? } else{ ?>
		<div   id="aviancahome"> <img src="<?=$img_a?>" width="100%"></div>
		<? } ?>
		<div   id="textohome">
			<p><b>BIENVENIDO</b> <br>
				<? if(plantilla==false){ ?>
...
<div id="gracias"  class="divContentBlue" style="display:none">
	<div id="textgracias">
	<p style="font-size: 24px;">Desde el SISSOMA agradecemos tu tiempo e interés. Tu encuesta ha sido diligenciada exitosamente.
	</p>
	</div>
 </div>	
