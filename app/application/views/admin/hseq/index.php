		...
		<div id="addUpdate">
			...
			<div class="divContentBlue" id="modal"><div id="titulo"><b>Nuevo</b></div>
			  <div class="divContentWhite"> 
 			  <?php echo CHtml::form(array("admin/hseq/action"), 'post', array('class'=>'eliminar', 'id'=>'eliminar')); ?>
				 <input type="hidden" name="option" id="option" value="delete" />
				 <input type="hidden" name="Ushseq[id]" id="id" value="" />
			  </form>
			  <?php echo CHtml::form(array("admin/hseq/action"), 'post', array('class'=>'formularioEstructura', 'id'=>'formForm')); ?>
 			  <input type="hidden" name="option" id="option" value="create" />
 			  <table cellpadding="0" cellspacing="0" border="0" width="100%">
			    <tr>
				  <td>Aplicar encuesta simplificada:</td>
				  <td><input type="checkbox" id="simplificar" checked="checked"  /> 
 					  <input type="hidden" id="simplificado" name="Ushseq[simplificado]" value="Y" />
 					  </td>
				</tr>
			    <tr>
				  <td>Nombre:</td>
				  <td><input type="text" id="nombre" name="Ushseq[nombre]" style="width:90%;" maxlength="200" />
 					  <input type="hidden" id="codigo" name="Ushseq[codigo]" value="" />
					  </td>
				</tr>
			    <tr>
				  <td>Organizacion:</td>
				  <td> 
						<?
						echo '<select name="Ushseq[idorg]" style="width:90%;"><option value="-1">Seleccione una empresa</option>';
			    ...
			    <tr>
				  ...
				  <td><input type="text" id="fechainicio" name="Ushseq[fechainicio]" style="width:90%;" maxlength="200" />
 					  </td>
				</tr>
				 
			    <tr>
				  <td>Fecha de finalización:</td>
				  <td><input type="text" id="fechafin" name="Ushseq[fechafin]" style="width:90%;" maxlength="200" />
 					  </td>
				</tr>
				 
			    <tr>
				  <td>Documentos de identificación<br> (sin puntos y separados con coma):</td>
				  <td><textarea  id="documentos" name="Ushseq[documentos]" style="    width: 90%; height: 100px;" ></textarea>
 					  </td>
				</tr> 
                  ...
                  <tr  style="border: 1px solid #EDEDFB;background-color: #EDEDFB">
					...
					$contador=0;
 					foreach($hseqdisp as $dato){
						...
						$organizacion=Organizacion::model()->findByPk($dato->idorg);
  						...
  						echo('<tr style="border: 1px solid #EDEDFB;background-color: '.$color.'">'
							...
							echo ('<div style="border: 3px solid #ededfb;border-radius: 8px;padding: 5px;">'.
							  ...
							  '<div style="text-align:left" id="organizacion:'.$dato->idorg.'"><b>Organización: '. ($organizacion->nombre).'</b>'.
							 '</div>'.
							 '<div style="text-align:left" id="nombre:'.$dato->id.'"><b>Envía este link a los colaboradores de tu organización, podrás visualizar la información consolidada en el link de reportes del módulo 4</b>'.
 							 '</div>'.
...
<script>
	...
	jQuery("[rel^='accion_']").click(function(){
		...
		switch(option){
			case "update":jQuery("#addUpdate").show();
				...
				if(jQuery(".formularioEstructura").find("[name='Ushseq[id]']").size()==0){
					jQuery(".formularioEstructura").append('<input type="hidden" name="Ushseq[id]" />');
				}
				jQuery.each(jsonupdate,function(k,d){
 					if(jQuery(".formularioEstructura").find("[name='Ushseq["+k+"]']").size()>0){
 						jQuery(".formularioEstructura").find("[name='Ushseq["+k+"]']").val(d);
						
					}
				});
				if(jQuery("#simplificar").is(":checked")){jQuery("[name='Ushseq[simplificado]']").val("Y")}
				...
				jQuery("#titulo").html("<b>Editar HSEQ</b>");
			...
			case "create":jQuery("#addUpdate").show();
				jQuery(".formularioEstructura").find("#option").val(option);
				jQuery(".formularioEstructura").find("[name^='Ushseq[']").val("");
				jQuery(".formularioEstructura").find("[name='Ushseq[id]']").remove();
				jQuery(".formularioEstructura").find("[rel^='b_']").hide();
			...
			case "delete":
				...
				var id=jQuery(this).attr("id").split("delete_")[1];
					jQuery(".eliminar").find("[name='Ushseq[id]']").val(id);
 		...
 		jQuery("[id^='b_']").click(function(){
			if(jQuery("[name='Ushseq[idorg]']").val()>0 && jQuery("#nombre").val()!="" && $( "#fechainicio").val()!="" && $("#fechafin" ).val()!=""){
	...
	jQuery("[rel='cancelar']").click(function(){
		...
		jQuery("#simplificar").click(function(){
			var chec=jQuery(this).is(":checked");
			if(chec){jQuery("#simplificado").val("Y")}else{jQuery("#simplificado").val("N")}
