...
function event(){ 
	...
	<? if(isset($_GET['organizacion'])){?>jQuery("[name='organizacion']").val(<?=(int)$_GET['organizacion']?>);<?} ?>
 	 jQuery("[name='nodo']").click(function(){
		...
		var urlbase="https://www.talentracking.co/node/"+window["nodo"]+"?embed=1&idestructuraSrt="+strEstructuras+"&token=<?=$token?>"+anio;
		...
		<h3><b><span id="r_titulo"></span></b></h3>
  		<div class="divContentWhite">
			...
			<br/>
			 ...
			 <!--div id="chart_div"></div-->
			<iframe id="reporte" name="reporte" src="https://www.talentracking.co/node/302?embed=1&idestructuraSrt=<?=$idestructura?>&token=<?=$token?>"  ></iframe>  
