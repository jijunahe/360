		...
		function selectHandler() {
			...
			if (selectedItem) {
				...
				var idpadre=datasplit[2];
 				var action="<?=Yii::app()->baseUrl?>/index.php/admin/hseq/analisisSociodemografico";
				...
				jQuery.ajax({ 
					...
					success:function(res){
						...
						//console.log(chart);
 						...
 						var urlbase="https://www.talentracking.co/node/160?embed=1&idestructuraSrt="+idestructura+"&paramEmpresaSrt=<?=$idorg?>&paramempresaSrt=<?=$idorg?>&paramempresa=<?=$idorg?>&token="+token+paramanioc;
						drawChart(chart[0]);
...
jQuery(function(){
	...
	jQuery("[name='anio']").change(function(){
		...
		var action="<?=Yii::app()->baseUrl?>/index.php/admin/rcv/reportesimplificado";
		...
		jQuery.ajax({ 
			...
			success:function(res){
				...
				var urlbase="https://www.talentracking.co/node/160?embed=1&idestructuraSrt="+idestructura+"&paramEmpresaSrt=<?=$idorg?>&paramempresa=<?=$idorg?>&token="+token+paramanioc;
 				drawChart(chart[0]);
				jQuery("#reporte").attr({"src":urlbase});
		...
		<h3>Reportes</h3>
  		<div class="divContentWhite">
			...
			<br/>
			 ...
			 <!--div id="chart_div"></div-->
			<iframe id="reporte" name="reporte" src="https://www.talentracking.co/node/160?embed=1&token=<?=$token?>&paramEmpresaSrt=<?=$idorg?>&paramempresa=<?=$idorg?>&idestructuraSrt=<?=$idestructura?>"  ></iframe>  
			<div style="clear:both"></div>
		</div>
 	</div>
	<div style="clear:both"></div>			 
