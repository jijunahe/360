	...
	<td style="padding:20px;border:1px solid #CCCCCC" align="left">
		...
		<div class="divContentBlue">Seleccione un periodo - Por defecto se muestran los datos del periodo actual
			  <div class="divContentWhite">
				...
				<select  id="sel_periodo"  style="border: 1px; border-style: solid; border-color:gray; font-size: 12px;">
					<option value=""  >
			...
			<div class="divContentBlue">Evaluaciones Realizadas: <?php echo($realfalt->totalReal)?>   --   Evaluaciones Por Realizar: <?php echo($realfalt->totalFalt)?>
			  ...
			  <table id="hor-minimalist-b" width="100%" border="0" cellspacing="0" cellpadding="0" >
                <tr>
				  ...
				  <td class="text" style="color:#1F497D;font-weight:bold;text-align: center;">Pais</td>
				  <td class="text" style="color:#1F497D;font-weight:bold;text-align: center;">Proceso</td>
                  <td class="text" style="color:#1F497D;font-weight:bold;text-align: center;">Proyecto</td>
				...
				for($i=0;$i<count($listaa);$i++){
					...
					if($dato->autoevalusu == 0){	
						$fila = $filatr.'<td valign="middle" style="text-align:left;" ">'.$imr.''.($dato->nombre).'</td>';
					}else{
						$fila = $filatr.'<td valign="middle" style="text-align:left;" ">'.$ime.''.($dato->nombre).'</td>';
					 }
					 $fila = $fila.'<td style="text-align:center;">'.($dato->pais).'</td>';
					 $fila = $fila.'<td style="text-align:center;">'.($dato->proceso).'</td>';
					 $fila = $fila.'<td style="text-align:center;">'.($dato->proyectos).'</td>';
					if($totaleva!=0){
					...
					//$tab2 = $tab2.'<td style="text-align:center;"><table style="width: 100%;">';
						for($j=0;$j<count($report);$j++){
							...
							if($dator->realeval == 1){
								...
								if($dator->realevalJefe == 1 && $dator->tipo == 3){
									$reval=1;
									$ima=$ime;
								}else{
									$ima=$imr;
							...
							if($dator->evalua==0){
								...
								$tab1 = $tab1.'<tr><td style="text-align:center; width: 170px;">'.$dator->rel2.'</td><td style="text-align:left; width: 270px;">'.$ima.''.($dator->nombGen).'</td></tr>';
							}
							//$tab2 = $tab2.'<tr><td style="text-align:left;">'.$ima.''.($dator->nombGen).'</td></tr>';
							
					    }
						$tab1 = $tab1.'</table></td></tr>';
						//$tab2 = $tab2.'</table></td>';
						$fila = $fila.$tab1;
						//$fila = $fila.$tab2;
					}else{
					...
					$fila = $fila.'</td>';
					if(($dato->autoevalusu == 0 ) || $totalcri == 0){
					$fila = $fila.'<td><b style="color:red;">x</b></td>'.
							 '<td><b style="color:red;">x</b></td>'.
						     '</tr>';
					 }else{
						$fila = $fila.'<td><a href="reporteCompetenciasComparativa.php?id='.$dato->id.'&periodo='.$perid.'">repComp</a></td>'.
						'<td><a href="reporteCompetenciasConsolidada.php?id='.$dato->id.'&periodo='.$perid.'">repCons</a></td>'.
						'</tr>';
					 }
					$fila = $fila.$filaeval;*/
					echo($fila);
