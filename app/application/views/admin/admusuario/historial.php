...
<script src="<?=Yii::app()->baseUrl?>/scripts/admin/colorbox/jquery.colorbox.js"></script>
	...
	<div id="page-wrapper">
            ...
            <!-- /.row -->
		<h4><strong>Historial de Ventas (Organizado por Fecha)</strong></h4>
		<br>
		<div class="row form-inline" style="margin: auto">
			<div class="col-md-4">
				<label for="fecha_inicio">Filtro de:</label>
				<input type="date" class="form-control" name="fecha_inicial" id="fecha_inicial" placeholder="DD/MM/YY">
			</div>
			<div class="col-md-4">
				<label for="fecha_final">a:</label>
				<input type="date" class="form-control" name="fecha_final" id="fecha_final" placeholder="DD/MM/YY">
			</div>
			<div class="col-md-4">
			...
			<table class="table table-bordered">
				<thead class="bg-primary">
				   <div class="row">
					...
					<th class="col-md-3">Cr&#233ditos Adicionados</th>
					<!--<th class="col-md-3">Cr&#233ditos Retirados</th>-->
				  </div>
				</thead>
				<tbody id="contenido">
	...
	<script>
		var ventas = <?=$ventas?>;
		jQuery.each(ventas,function(key,value){
		contenido += '<tr><div class="row">'+
			'<td class="col-md-3">'+value.fecharegistro+'</td>'+
			'<td class="col-md-3">'+value.usuario.nombres+'</td>'+
			'<td class="col-md-3 text-center">'+value.creditos+'</td>'+
			'</tr></div>';
		});
		...
		jQuery("#buscar").click(function(){
			...
			if(fecha_inicial == "" | fecha_final == ""){
				jQuery.colorbox({html:"<center><h2>Los campos de Fecha no pueden estar vacios</h2></center>"});
				return;	
			}
			jQuery.post("<?=Yii::app()->baseUrl?>/index.php/admin/admusuario/filtroFecha",{fecha_inicial:fecha_inicial,fecha_final:fecha_final},function(response){
				response = JSON.parse(response);
				if(response.length == 0){
					jQuery.colorbox({html:"<center><h2>No Exiten Resultados Para Las fechas Ingresadas</h2></center>"});
					return;
				}
				var contenido = "";
