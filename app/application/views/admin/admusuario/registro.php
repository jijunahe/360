		...
		submitHandler: function() {
			...
			success:function(res){
				...
				jQuery("input[type='email']").val("");
 				if(res[0]=="OK"){
					jQuery.colorbox({html:"<h3>El usuario ha sido creado exitosamente, por favor revise su email para confirmar su registro</h3>"});
				}else if(res[0]=="existe"){
					jQuery.colorbox({html:"<h3>El usuario ya existe, por favor haga click <a href='<?=$_SERVER["SCRIPT_NAME"]?>/admin/authentication/sa/recuperar'>Aquí</a> para recuperar su acceso</h3>"});
				}else{
 ...
 <div id="content">
	...
	<td style="padding:20px;border:1px solid #CCCCCC" align="left">
		...
		<td>
			...
			<div class="divContentBlue"><b><?=$org->nombre?></b>
			  <div class="divContentWhite">
				<?php echo CHtml::form(array("admin/authentication/sa/registro"), 'post', array('class'=>'formCrear' , 'id'=>'formCrear','style'=>'margin:0px; padding:0px;')); ?>
			    <input type="hidden" name="save" value="1" />
				<table width="100%" border="0" cellspacing="0" cellpadding="2" id="tableData">
  				  <tr valign="top">
					 <td><b>Nombre y apellido</b></td>
					 <td>
 						<input type="text" name="nombres" id="nombres" <? if(is_object($usuario)){?>value="<?=$usuario->nombres?>"<?}?> maxlength="255" style="width:100px" >
  					 </td>
				  </tr>
