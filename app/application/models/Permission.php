...
class Permission extends LSActiveRecord
{
    ...
    */
	...
	public function getanOterpermision($validar=NULL){
	 
		$oRecord = User::model()->findByPk($_SESSION['loginID']); 
		//printVar($oRecord);
		$dUsuario = AnaUsuario::model()->findByPk($oRecord->iduseval);	 
		$permisos=$this->getPermissions((int)Yii::app()->user->id); //printVar($permisos);
		...
		$esadministrador=false;
 		if($totalPermisos>=1){
			if(!isset($dUsuario->perfil)){
				...
				$bandera=false;
				$bansera2=false;
				foreach($permisos as $data){ //printVar($data["permission"]);
					if($data["permission"]=="superadmin"){
						$bandera=true;
						$bansera2=true;
					}				
				}
	  
				if($bandera==true and $bansera2==true){
					$esadministrador=true;
				}
			}else if(($dUsuario->perfil==3 or $dUsuario->perfil==1) and $validar=="administrador"){
				$esadministrador=true;
 			}
  		}else if($permisos[0]["permission"]=="superadmin"){
					$esadministrador=true;
  		} 
