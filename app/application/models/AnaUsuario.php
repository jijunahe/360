 ...
 * @property integer $idorgmenuserv
...
class AnaUsuario extends CActiveRecord
{
	...
	public function rules()
	{
		...
		return array(
			...
			array('perfil, id_area, id_cargo, activo, idorgmenuserv, id_pais, id_region, id_ciudad', 'numerical', 'integerOnly'=>true),
			array('nombres', 'length', 'max'=>255),
			array('id_unidad, alias', 'length', 'max'=>200),
			...
			array('id, tokenid, perfil, nombres, id_unidad, id_area, id_cargo, alias, clave, documento, email, uid_creador, activo, fecharegistro, idorgmenuserv, id_pais, id_region, id_ciudad', 'safe', 'on'=>'search'),
		);
	}
	...
	function vaidausr($id){
		...
		$msg="NO";
    	if(($user[0]["perfil"]==1 or $user[0]["perfil"]==2) or Permission::model()->getanOterpermision()){
			$msg= "OK";
 		}
		else{
			if($par_id!=false){	
	...
	public function search()
	{
		...
		$criteria->compare('idorgmenuserv',$this->idorgmenuserv);
